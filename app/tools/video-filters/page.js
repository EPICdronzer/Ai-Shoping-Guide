...
export default function VideoFilters() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setProgress(100);
      setResult({ blob, size: blob.size, name: file.name.replace(/\.[^.]+$/, `_${selectedFilter.id}.${exportFormat}`) });
    ...
    <ToolLayout icon="🎨" title="Video Filters" category="Editing Tools" badgeColor="#e879f9">
      ...
      {result && (
        <div style={{ ...card, border: '1px solid rgba(232,121,249,0.3)' }}>
          ...
          <button onClick={download} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#059669,#10b981)', color: '#fff', fontSize: '15px', fontWeight: '700', marginBottom: '10px' }}>⬇️ Download Filtered Video (.${exportFormat})</button>
