...
export default function VideoTrimmer() {
  ...
  const handleTrim = async () => {
    ...
    try {
      ...
      setProgress(100);
      setResult({ blob, size: blob.size, name: file.name.replace(/\.[^.]+$/, '_trimmed.webm') });
    ...
    <ToolLayout icon="✂️" title="Video Trimmer" category="Basic Video Tools" badgeColor="#34d399">
      ...
      {result && (
        <div style={{ ...card, border: '1px solid rgba(52,211,153,0.3)' }}>
          ...
          <button onClick={download} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#059669,#10b981)', color: '#fff', fontSize: '15px', fontWeight: '700', marginBottom: '10px' }}>⬇️ Download Trimmed Video (.webm)</button>
