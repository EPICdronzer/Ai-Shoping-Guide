...
export default function MuteVideo() {
  ...
  const handleMute = async () => {
    ...
    try {
      ...
      setProgress(100);
      setResult({ blob, size: blob.size, name: file.name.replace(/\.[^.]+$/, `_muted.${exportFormat}`) });
    ...
    <ToolLayout icon="🔇" title="Mute Video" category="Basic Video Tools" badgeColor="#f472b6">
      ...
      {result && (
        <div style={{ ...card, border: '1px solid rgba(244,114,182,0.3)' }}>
          ...
          <button onClick={download} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#059669,#10b981)', color: '#fff', fontSize: '15px', fontWeight: '700', marginBottom: '10px' }}>⬇️ Download Muted Video (.${exportFormat})</button>
