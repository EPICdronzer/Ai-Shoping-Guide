...
export default function AddPdfWatermark() {
  ...
  const handleWatermark = async () => {
    if (!bufRef.current || !watermark) return;
  ...
  return (
    <ToolLayout icon="💧" title="Add Watermark" category="PDF Tools" badgeColor="#fb923c">
      <div style={card}>
        <div onDrop={onDrop} onDragOver={e => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)}
          onClick={() => !file && inputRef.current?.click()}
          style={{ border: `2px dashed ${isDragging ? 'rgba(251,146,60,0.8)' : 'rgba(251,146,60,0.3)'}`, borderRadius: '12px', padding: '32px', textAlign: 'center', cursor: file ? 'default' : 'pointer', transition: 'all 0.25s' }}>
      ...
      {file && (
        <div style={card}>
          <label style={{ fontSize: '13px', color: '#94a3b8', display: 'block', marginBottom: '8px' }}>WATERMARK TEXT</label>
          <input
            type="text"
            value={watermark}
      ...
      {file && watermark && !done && (
        <button onClick={handleWatermark} disabled={processing} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#c2410c,#f97316)', color: '#fff', fontSize: '15px', fontWeight: '700' }}>
          {processing ? '⏳ Drawing…' : '💧 Add Watermark & Download'}
        </button>
      )}
      ...
      {done && (
        <div style={{ ...card, border: '1px solid rgba(52,211,153,0.3)', textAlign: 'center', background: 'rgba(16,185,129,0.05)' }}>
          ...
          <p style={{ color: '#34d399', fontWeight: '700', marginTop: '8px' }}>PDF watermarked and downloaded successfully!</p>
        </div>
      )}
    </ToolLayout>
  );
